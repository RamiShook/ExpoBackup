    ...
    <div class="mdl-layout mdl-js-layout mdl-layout--fixed-header">
      <header class="mdl-layout__header">
        <div class="mdl-layout__header-row">
          <!-- Title -->
          <span class="mdl-layout-title">Replacement Order's</span>
...
<!--        &nbsp; &nbsp; &nbsp;Search By Name: <input type="text" id="NameInput" onkeyup="SearchByName()" placeholder="Search By Name.." title="Type The Product Name"> -->
    ...
    $wkid=$_SESSION['uid'];
     ...
     SELECT * FROM (Relacement_details,Replacement)
INNER JOIN products ON (products.product_id = Relacement_details.new_pid)
WHERE Replacement.mres_id=Relacement_details.mres_id")or die("error");
     
     /*
     select multiple_reserved_product.multiple_reserve_id,
...
$PriceCalc=0;
  while($row = mysqli_fetch_assoc($q)){
    $thervid = $row['mres_id'];
      //
      $queryz = "SELECT count(*) as counts from Relacement_details WHERE Relacement_details.mres_id= $thervid";
    $countquery=mysqli_query($connection, $queryz)or die(mysqli_error($connection));;
    $data=mysqli_fetch_assoc($countquery);
    
//query to get old product
$oldpidx = $row['old_pid'];
//query to get old product id
$qoldp = "SELECT product_Code FROM products where products.product_id = '$oldpidx' ";
$result11=mysqli_query($connection,$qoldp)or die(mysqli_error($connection));

$dataaz = mysqli_fetch_assoc($result11);
//The Client Id That Have This Phone Number.
$Poduct_Code = $dataaz['product_Code'];

    echo '<tr id="tr'.$row['mres_id'].'">
    <td>'.$row['mres_id'].'</td>
    <td>'.$row['product_Code'].'</td>
        <td>'.$row['new_p_qty'].'</td>
        <td>'.$row['new_pprice'].'</td>
        
        ';
        if ($row['RD_Check_Status']==0){
echo '<td><input type="checkbox" onclick = "SetCheck(1,'.$row['mres_id'].', '.$row['new_pid'].',this)" disabled /></td>';
}else{
  echo ' <td> <input type="checkbox" checked="true" onclick = "SetCheck(0,'.$row['mres_id'].' , '.$row['new_prid'].',this)" disabled/></td>';
}

        //eza ma ken fi ella wahde 
       if($data['counts'] == $rowcounter){ 
           echo '</tr> <tr> <td colspan=3 color="red">The following Product Is Returned Or Replaced By The Above Product\'s Delivery Charge: '.$row['delivery_charge'].'</td> <td> <input type="button" value="PRINT THIS ORDER" onclick="printod('.$row['mres_id'].')"></button></td></tr>
           <tr><td>'.$row['mres_id'].' SHP_NB:'.$row['shipment_number'].' </td> <td> <u>'.$Poduct_Code.' </td> <td> <u>'.$row['old_p_qty'].' </td> <td> <u>'.$row['old_pprice'].'</td>';
           if($row['Return_Status']==0){
           echo '<td> <input type="button" value="Return To Stock" onclick="RetunStock('.$row['old_pid'].','.$row['old_p_qty'].','.$row['mres_id'].',this)"></input></td>';
           }else
           echo '<td> <input type="button" value="Already Returned!" onclick="RetunStock('.$row['old_pid'].','.$row['old_p_qty'].','.$row['mres_id'].')" disabled></input></td>';
...
function RetunStock(pid,pqty,mres,thbtn){
    
    console.log("the old product id :"+pid);
    console.log("The Old Product Quantity"+pqty);
...
xmlhttp.open("GET","./ReplacementFunctions.php?ReturnSTK="+pid+"&qt="+pqty+"&mres="+mres,false);
xmlhttp.send();   
thbtn.disabled=true;
thbtn.value="Returned!";

}

function printod(x){
   
var url = "https://cedarslandshoes.com/expo/PRNT/V176/Classes/returnmy.php?oid="+x;
...
xmlhttp.open("GET","./Fulfillment/FulfillmentFunctions.php?RDSetChecked="+checkedz+"&Ordrid="+Mid+"&prodid="+prodid,false);
xmlhttp.send();   
