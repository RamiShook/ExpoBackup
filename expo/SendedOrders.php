    ...
    <div class="mdl-layout mdl-js-layout mdl-layout--fixed-header">
      <header class="mdl-layout__header">
        <div class="mdl-layout__header-row">
          <!-- Title -->
          <span class="mdl-layout-title">Sent Orders</span>
...
<!--        &nbsp; &nbsp; &nbsp;Search By Name: <input type="text" id="NameInput" onkeyup="SearchByName()" placeholder="Search By Name.." title="Type The Product Name"> -->
    ...
    $wkid=$_SESSION['uid'];
     ...
     INNER JOIN workers ON workers.worker_id = multiple_reserve_worker_id WHERE multiple_reserve_status = 'Sent'
    ...
    <div class="">
        <div class="panel panel-default">
          <div class="panel-heading no-collapse">  <center>Total Sended Orders: <span class="label label-warning" id="rescc"></center></span></div>
    ...
    $PriceCalc = $PriceCalc+ $row['price'] ;
              ...
              echo '<tr id="tr'.$row['multiple_reserve_id'].'">
                  ...
                  if($data['counts'] == $rowcounter){
                      ...
                      echo "<u>The Above Order Is For The Client: </u><strong>".$row['client_FullName']."</strong>&nbsp; &nbsp; <u> Phone:</u><strong>".$row['client_Phone']."</strong><br><u>The Total Price Is</u>:<strong> ".$PriceCalc."</strong> <br>Order Date:<strong>".$row['reserve_date']."</strong>
                      <br> Order Status: ".$row['multiple_reserve_status']."<br>
                      Reserved By The Worker: <strong>".$row['worker_Full_Name']." </strong><br> note: ".$row['multiple_reserve_note']." <br> Shipement NB :".$row['shipment_number']." </strong><br><hr></td>";
                      $rowcounter=1;
                      $PriceCalc=0;
                    ...
                    if($row['multiple_reserve_status'] == 'Sent'){
                        echo '<br><input type="button" id='.$row['multiple_reserve_id'].' onclick="confirmrv(this.id,this)" value="Approve" ></td> </tr>';
                    }else { echo '
                    <br><input type="button" id='.$row['multiple_reserve_id'].' onclick="confirmrv(this.id,this)" value="Approve" disabled=true></td> </tr>';
                    }
...
xmlhttp.open("GET","./Functions.php?SetApproved="+x,true);
xmlhttp.send();   
