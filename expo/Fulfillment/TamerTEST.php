...
multiple_reserved_product.Check_Status,
     ...
     WHERE multiple_reserved.multiple_reserve_status= 'Fulfillment' AND multiple_reserved.multiple_reserve_worker_id IN(1,7) AND Eliminated = 0
 ORDER BY multiple_reserved.reserve_date DESC ")or die("error".mysqli_error($connection));
     $t = mysqli_num_rows($q);
...
// knowing if this product is in minus qty

$ppcode = $row['product_Code'];
$CheckProductQtyTXT = "select product_Quantity from products where product_Code = '$ppcode' ";
$ProductQty = mysqli_query($connection,$CheckProductQtyTXT);
$ProductCheckQty = mysqli_fetch_assoc($ProductQty);
...
//if the product qty is in negative qty then td backgroud must be red
if($ProductCheckQty['product_Quantity'] <0 ){
    echo'<td bgcolor="red" >'.$row['product_Code'].' </td>';
  }             else {
...
echo '<td><input type="checkbox" onclick = "SetCheck(1,'.$row['multiple_reserve_id'].', '.$row['multiple_reserve_product_id'].',this)" /></td>';
}else{
  echo ' <td> <input type="checkbox" checked="true" onclick = "SetCheck(0,'.$row['multiple_reserve_id'].' , '.$row['multiple_reserve_product_id'].',this)"/></td>';
}
...
echo"5000 Delivery";
        ...
        echo "4000 Delivery";
                   ...
                   echo '
                   <td><input type="button" value="ELiminate This Order" onclick="Eliminate('.$row['multiple_reserve_id'].',this)" ></input></td>
                   ...
                   <br><br><input type="button" id="btn'.$row['multiple_reserve_id'].'" name="'.$row['multiple_reserve_id'].'"  value="Print This Order" onclick="PrintZ(this)">
...
// to eliminate the order to the waiting page ,
function Eliminate(ordrid,ths){
...
ths.value="Eliminated";
ths.disabled=true;
}
}
xmlhttp.open("GET","./FulfillmentFunctions.php?Eliminate="+ordrid,false);
xmlhttp.send();
...
var url = "https://cedarslandshoes.com/expo/PRNT/V176/Classes/my1BIGFONT.php?oid="+OrdrId;
