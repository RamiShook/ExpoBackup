...
echo '<td><input type="checkbox" onclick = "SetCheck(1,'.$row['mres_id'].', '.$row['new_pid'].',this)" /></td>';
}else{
  echo ' <td> <input type="checkbox" checked="true" onclick = "SetCheck(0,'.$row['mres_id'].' , '.$row['new_prid'].',this)"/></td>';
}

        //eza ma ken fi ella wahde 
       if($data['counts'] == $rowcounter){ 
           echo '</tr> <tr> <td colspan=3 color="red">The following Product Is Returned Or Replaced By The Above Product\'s</td> <td> <input type="button" value="PRINT THIS ORDER" onclick="printod('.$row['mres_id'].')"></button></td></tr>
           <tr><td>'.$row['mres_id'].' SHP_NB:'.$row['shipment_number'].' </td> <td> <u>'.$row['old_pid'].' </td> <td> <u>'.$row['old_p_qty'].' </td> <td> <u>'.$row['old_pprice'].'   
           </tr> <td colspan=5>/---------------------------------------------------------------------/';
           $rowcounter=1;
       }else{
                      $rowcounter ++;
...
xmlhttp.open("GET","./FulfillmentFunctions.php?RDSetChecked="+checkedz+"&Ordrid="+Mid+"&prodid="+prodid,false);
xmlhttp.send();   
