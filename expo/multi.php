...
var clid= <?php echo $ClientId; ?> ;
var wkid= <?php echo $WorkerId; ?> ;
...
xmlhttp.open("GET","./Functions.php?mlrvcid="+clid+"&mrwid="+wkid,false);
xmlhttp.send();
 } 
...
<div id="mydiv" align="center"><img src="assets/wait.gif" class="ajax-loader"></div>   </div>
    ...
    <div class="">
        ...
        <?php 
         ...
         $data = mysqli_fetch_assoc($result);
         echo "<hr><strong>You Are Now Dealing With: ".$data['client_FullName']."</strong>" ;   
         echo "<BR><strong>Client Phone: <div id='CPH'>".$_GET['client']."</div></strong>" ;   
echo "<hr>"
...
?>
        ...
        <div id="cont"> </div>
        <div > The Full Price Of Confirmed Order in This Reservation: <div id="fprc">0</div> </div>
        <input type="button" value="Confirm All" onclick="cnfAll()"> </input>
      ...
      document.getElementById('TabsCount').innerHTML= ++Tab;
        ...
        for (var c = 0; c < arrHead.length; c++) {
            ...
            //Phone Number
                ...
                ele.setAttribute('value',''+document.getElementById('CPH').innerHTML);
...
function unreserve(x,y){
    rowId=y;
RowClientPhone=document.getElementById('ClientPhoneNumber'+y).value;
RowProductCode = document.getElementById('ProductCode'+y).value;
RowQuantity = document.getElementById('Quantity'+y).value;
//RowNotes = document.getElementById('Notes'+rowId).value;
RowPrice= document.getElementById('Price'+y).value;
...
xmlhttp.open("GET","./Functions.php?rmrowid="+y+"&rspcode="+pccode+"&qty="+qyy+"&mrClientPhone="+RowClientPhone,false);
xmlhttp.send();
...
rowId=TheId;
RowClientPhone=document.getElementById('CPH').innerHTML;
console.log("ClientPhone= "+RowClientPhone);
...
RowPrice= document.getElementById('Price'+rowId).value;
  ...
  document.getElementById("TotalReservation").innerHTML=counter;
 wrkrid  =<?php echo $_SESSION['uid']; ?> ;
 console.log("The Worker Id = "+wrkrid);
...
(function(){
    ...
    var data = {message: 'The System Said: ' +xmlhttp.responseText};
...
var FullPrc = document.getElementById('fprc').innerHTML;
xmlhttp.open("GET","./Functions.php?mrClientPhone="+RowClientPhone+"&mrProductCode="+RowProductCode+"&mrQuantity="+RowQuantity+"&mrPrice="+RowPrice+"&rwid="+rowId+"&WKRID="+wrkrid+"&RND="+timestamp+"&FullPrice="+FullPrc,false);
xmlhttp.send();

// first disable the confirm button // to make sure the user dont confirm twice
...
// show and enable the unreserve button
document.getElementById('multi'+rowId).hidden=false;
document.getElementById('multi'+rowId).disabled=false;

// disable the product code input
...
function cnfAll(){
    if(counter == 0 || Tab==0){
        alert("Confirm What!\nPlease Fill The Order First.")
    }
    else if(counter != Tab ){
        alert("Please Make Sure That All The Order Row's Reserved First!")
    }else{ 

close=1;
alert("All Right! Now You Can Close This Tab \nIgnore The Alert When You Close.")
} }
