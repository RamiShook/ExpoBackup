...
multiple_reserve_status,
     ...
     multiple_reserved.multiple_reserve_status= 'Fulfillment' OR multiple_reserved.multiple_reserve_status= 'Sended')  ORDER BY multiple_reserved.reserve_date DESC ")or die("error");
     $t = mysqli_num_rows($q);
    ...
    <div class="">
        <div class="panel panel-default">
          <div class="panel-heading no-collapse">  <center>Total Orders: <span class="label label-warning" id="rescc"></span>
           <br><span class="label label-warning"><b> Note That All The Orders Here Was Sended To The Fulfillment But Not Sended To The Client </b></span>
          </center></div>
...
echo '<td><input type="checkbox" id='.$row['Check_Status'].'"  disabled/></td>';
}else{
  echo ' <td> <input type="checkbox" checked="true" disabled /></td>';
}
echo '<td> <input type="button" value="Remove This From Order" onclick="RemoveTHIS(\''.$row['multiple_reserve_id'].'\',\''.$row['product_id'].'\',\''.$row['quantity'].'\')"> </input>';
echo'
                  ...
                  if($data['counts'] == $rowcounter){
                      ...
                      echo "<u>The Above Order Is For The Client: </u><strong>".$row['client_FullName']."</strong>&nbsp; &nbsp;
                       <u> Phone:</u><strong>".$row['client_Phone']."</strong><br><u>The Total Price Is</u>:<strong> ".$PriceCalc." <u>Not</u> Contain The Delivery Charge.</strong> <br>Order Date:<strong>".$row['reserve_date']."</strong>
                        <br><u> Address:</u><strong>".$row['client_Address']."</strong> 
                        <BR> Shipment Number: <strong>".$row['shipment_number']."
                        <br> note: ".$row['multiple_reserve_note']."</strong><br> 
                        <br> Status : ".$row['multiple_reserve_status']."</strong><hr></td>";
                      $rowcounter=1;
                      $PriceCalc=0;
                      $mutiplereserveid = $row['multiple_reserve_id'];
                    echo ' <td> <input type="button" id="'.$mutiplereserveid.'" onclick="del(this.id,this)" value="Delete This Order"></input> 
                    <BR><br><br> <input type="button"  onclick="AddProducts(\''.$row['multiple_reserve_id'].'\',this)" value="Add Products"></input> 
                    <br> <br> <br></td></tr>
                    ';
...
function RemoveTHIS(mrid,pid,pqty){
    cnf=confirm("This Will Delete The Selected Product From The Order, Please Make Sure This Product Is \"Not Checked\" \n AND DONT FROGET TO TELL FULFILLMENT ABOUT THIS STEP! \n DONOT FORGETTTTT!😝");
                        if(!cnf) return;
...
xmlhttp.open("GET","./Functions.php?delMulResProduct="+mrid+"&pid="+pid+"&pqty="+pqty,false);
xmlhttp.send();   
                  ...
                  function del(x,ths){
                    ...
                    cnf=confirm("This Will Delete This Order With All Product That Have !\nPlease Make Sure You Want To Delete The Order Id:"+btnid+"\nKHABRO L FULFILLMENT KRMEEEEEL ALLAH 😂")
                    if(!cnf) return;
                 var xmlhttp;
if(window.XMLHttpRequest)
xmlhttp = new XMLHttpRequest(); 
...
removeRow(btnid,ths); 
  ...
  function removeRow(btnid,oButton) {
    oButton.value="This Order Was Deleted!";
    oButton.parentNode.innerHTML="This Order Was Deleted! \nRefresh The Page To Remove It";
        var empTab = document.getElementById('productTable');
        //empTab.deleteRow(oButton.parentNode.parentNode.rowIndex); // button -> td -> tr.
    }
                  </script>
<script>
function AddProducts(mrid){
    window.open("https://cedarslandshoes.com/expo/AddPForExistMRES.php?mresid="+mrid);
