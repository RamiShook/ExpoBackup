...
<!--        &nbsp; &nbsp; &nbsp;Search By Name: <input type="text" id="NameInput" onkeyup="SearchByName()" placeholder="Search By Name.." title="Type The Product Name"> -->
    ...
    $wkid=$_SESSION['uid'];
     ...
     where multiple_reserved.multiple_reserve_status = 'Sent' AND  (CHAR_LENGTH(shipment_number)<3 AND shipment_number !='/')
  ...
  while($row = mysqli_fetch_assoc($q)){
    ...
    $PriceCalc = $PriceCalc+ $row['price'] ;
              ...
              echo '<tr id="tr'.$row['multiple_reserve_id'].'">
                  ...
                  <td>'.$row['quantity'].'</td>
                  <td>'.$row['price'].'</td>
                  <td> <input type="button" value="Restore QTY" onclick="returnqty(\''.$row['product_Code'].'\',\''.$row['quantity'].'\',this)" > </input></td>
...
function returnqty(pcode,pqty,ths){
...
ths.value=xmlhttp.responseText;
ths.disabled=true;
}
xmlhttp.open("GET","./Functions.php?RestoreQTY="+pcode+"&pqty="+pqty,false);
xmlhttp.send();   
}
                  function del(x,ths){
                    btnid=x;
